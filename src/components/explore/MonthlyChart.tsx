...
import type { ShutoffRecord, NationalMonthly } from '../../data/shutoffs-types';
...
export default function MonthlyChart({ stateMonthly, nationalMonthly, stateName, stateCode }: Props) {
  const [fuel, setFuel] = useState<Fuel>('electric');

  const sorted = [...stateMonthly].sort((a, b) => a.month - b.month);
  const natSorted = [...nationalMonthly].sort((a, b) => a.month - b.month);
  ...
  if (fuel === 'electric') {
    ...
    natVals = natSorted.map((r) => r.electric_shutoffs_total);
  } else if (fuel === 'gas') {
    stateVals = sorted.map((r) => r.gas_shutoffs ?? 0);
    natVals = natSorted.map((r) => r.gas_shutoffs_total);
  } else {
    stateVals = sorted.map((r) => r.electric_monthly_shutoff_rate);
    stateVals2 = sorted.map((r) => r.gas_monthly_shutoff_rate);
    natVals = natSorted.map((r) => r.electric_national_shutoff_rate);
  }

  const allVals = [...stateVals, ...natVals, ...(stateVals2 ?? [])];
  const maxVal = Math.max(...allVals, 0.0001);

  const statePts = toPoints(stateVals, maxVal);
  const natPts = toPoints(natVals, maxVal);
  ...
  const titleText = fuel === 'both'
    ? `Monthly electric and gas shutoff rates in ${stateName}, 2024, compared to the national average`
    : `Monthly ${fuel} shutoffs in ${stateName}, 2024, compared to the national average`;
    ...
    <div className="bg-[--color-surface] border border-[--color-border-light] rounded-xl px-6 py-5 mb-6">
      <div className="flex justify-between items-baseline mb-1">
        ...
        <div className="flex gap-4 text-xs text-[--color-text-secondary]">
          ...
          <span className="inline-flex items-center gap-1.5">
            <span className="inline-block w-3.5 h-0.5 border-t border-dashed border-[--color-neutral-bar]" />
            National avg
          </span>
          {fuel === 'both' && (
            <span className="inline-flex items-center gap-1.5">
